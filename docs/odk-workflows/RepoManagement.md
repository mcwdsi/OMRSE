...
1. Add an import statement to your `src/ontology/omrse-edit.owl` file. We suggest to do this using a text editor, by simply copying an existing import declaration and renaming it to the new ontology import, for example as follows:
    ```
    ...
...
2. Add your imports redirect to your catalog file `src/ontology/catalog-v001.xml`, for example:
    ```
    <uri name="http://purl.obolibrary.org/obo/omrse/imports/go_import.owl" uri="imports/go_import.owl"/>
    ```
...
in the catalog file to redirect the URL `http://purl.obolibrary.org/obo/omrse/imports/go_import.owl`
...
The following change to your repo config (`src/ontology/omrse-odk.yaml`) will switch the go import from an SLME module to a simple ROBOT filter module:
...
Now add a new goal in your custom Makefile (`src/ontology/omrse.Makefile`, _not_ `src/ontology/Makefile`).
...
1. Open `src/ontology/omrse-odk.yaml`
...
1. Providing a `components/mycomp.owl:` make target in `src/ontology/omrse.Makefile`
...
1. Providing an additional attribute for the component in `src/ontology/omrse-odk.yaml`, `source`,
